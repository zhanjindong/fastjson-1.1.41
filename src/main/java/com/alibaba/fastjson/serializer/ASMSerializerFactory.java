...
public class ASMSerializerFactory implements Opcodes {
	...
	public ObjectSerializer createJavaBeanSerializer(Class<?> clazz, Map<String, String> aliasMap) throws Exception {
		...
		// mw.visitFieldInsn(PUTFIELD, context.getClassName(),
		// fieldInfo.getName() + "_asm_prefix__", "[C");

		for (FieldInfo fieldInfo : getters) {
			mw.visitVarInsn(ALOAD, 0);

			// 如：Test.class push到栈顶
...

		{
			...
			mw = cw.visitMethod(
					ACC_PUBLIC,
					"write",
					"(Lcom/alibaba/fastjson/serializer/JSONSerializer;Lcom/alibaba/fastjson/serializer/FieldSerializer;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/reflect/Type;)V",
					null, new String[] { "java/io/IOException" });

			mw.visitVarInsn(ALOAD, context.serializer()); // 将第一个参数serializer
															// push 到栈顶
			mw.visitMethodInsn(INVOKEVIRTUAL, getType(JSONSerializer.class), "getWriter", "()"
					+ getDesc(SerializeWriter.class));// 调用serializer的getWriter
			mw.visitVarInsn(ASTORE, context.var("out"));// 将getWriter返回值(Object对象)从栈顶保存到局部变量中
			...
			if (jsonType == null || jsonType.alphabetic()) {
				...
				mw.visitFieldInsn(GETSTATIC, getType(SerializerFeature.class), "SortField", "L"
						+ getType(SerializerFeature.class) + ";");
				mw.visitMethodInsn(INVOKEVIRTUAL, getType(SerializeWriter.class), "isEnabled", "(" + "L"
						+ getType(SerializerFeature.class) + ";" + ")Z");
			...
			mw.visitInsn(RETURN);
			mw.visitMaxs(6, context.getVariantCount() + 1);// modify by
															// jdzhan:5->6
			mw.visitEnd();
		}
		...
		// java.io.FileOutputStream("D://temp/asm/" + className + ".class"));
	...
	private void generateWriteMethod(Class<?> clazz, MethodVisitor mw, List<FieldInfo> getters, Context context)
...

		{
			...
			mw.visitVarInsn(ALOAD, context.var("out"));
			mw.visitFieldInsn(GETSTATIC, getType(SerializerFeature.class), "PrettyFormat", "L"
					+ getType(SerializerFeature.class) + ";");
			mw.visitMethodInsn(INVOKEVIRTUAL, getType(SerializeWriter.class), "isEnabled", "(" + "L"
					+ getType(SerializerFeature.class) + ";" + ")Z");
			...
			mw.visitVarInsn(ALOAD, 3);
			mw.visitVarInsn(ALOAD, 4);
			mw.visitVarInsn(ALOAD, 5);// add by jdzhan
			mw.visitMethodInsn(
					INVOKEVIRTUAL,
...

		{
			...
			mw.visitFieldInsn(GETFIELD, context.getClassName(), "nature", getDesc(JavaBeanSerializer.class));
			mw.visitVarInsn(ALOAD, 1);
			mw.visitVarInsn(ALOAD, 3);// modify by jdzhan:2->3
		...
		// SEPERATO
		{
			...
			// mw.visitVarInsn(ALOAD, context.var("out"));
			// mw.visitFieldInsn(GETSTATIC, getType(SerializerFeature.class),
			// "WriteClassName",
			// "L" + getType(SerializerFeature.class) + ";");
			// mw.visitMethodInsn(INVOKEVIRTUAL, getType(SerializeWriter.class),
			// "isEnabled",
	...
	private void _list(Class<?> clazz, MethodVisitor mw, FieldInfo property, Context context) {
...

		{
			...
			mw.visitTypeInsn(CHECKCAST, getType(ObjectSerializer.class)); // cast
																			// to
																			// string
			...
			if (elementType == String.class) {
				...
				mw.visitTypeInsn(CHECKCAST, getType(String.class)); // cast to
																	// string
				mw.visitVarInsn(BIPUSH, ',');
				mw.visitMethodInsn(INVOKEVIRTUAL, getType(SerializeWriter.class), "writeString",
						"(Ljava/lang/String;C)V");
	...
	private void _filters(MethodVisitor mw, FieldInfo property, Context context, Label _end) {
		if (property.getField() != null) {
			if (Modifier.isTransient(property.getField().getModifiers())) {
				mw.visitVarInsn(ALOAD, context.var("out"));
				mw.visitFieldInsn(GETSTATIC, getType(SerializerFeature.class), "SkipTransientField", "L"
						+ getType(SerializerFeature.class) + ";");
				mw.visitMethodInsn(INVOKEVIRTUAL, getType(SerializeWriter.class), "isEnabled", "(" + "L"
						+ getType(SerializerFeature.class) + ";" + ")Z");
	...
	private void _if_write_null(MethodVisitor mw, FieldInfo fieldInfo, Context context) {
		...
		if (!writeNull) {
			mw.visitVarInsn(ALOAD, context.var("out"));
			mw.visitFieldInsn(GETSTATIC, getType(SerializerFeature.class), "WriteMapNullValue", "L"
					+ getType(SerializerFeature.class) + ";");
			mw.visitMethodInsn(INVOKEVIRTUAL, getType(SerializeWriter.class), "isEnabled", "(" + "L"
					+ getType(SerializerFeature.class) + ";" + ")Z");
			mw.visitJumpInsn(IFEQ, _else);
